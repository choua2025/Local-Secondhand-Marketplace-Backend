...
async function insertUser(client, input) {
    const { rows } = await client.query(`INSERT INTO users (email, password_hash, display_name, city, avatar_url)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`, [
        input.email,
        input.password_hash,
    ...
    const row = rows[0];
    if (!row)
        throw new Error(`Failed to insert user ${input.email}`);
    return row.id;
}
async function insertCategory(client, name, slug, parentId) {
    const { rows } = await client.query(`INSERT INTO categories (name, slug, parent_id) VALUES ($1, $2, $3) RETURNING id`, [name, slug, parentId]);
    const row = rows[0];
    if (!row)
        throw new Error(`Failed to insert category ${slug}`);
    return row.id;
}
/**
 ...
 */
let ageHours = 0;
async function insertListing(client, input) {
    ageHours += 1;
    const { rows } = await client.query(`INSERT INTO listings (seller_id, category_id, title, description, price, condition, city,
    ...
    const row = rows[0];
    if (!row)
        throw new Error(`Failed to insert listing ${input.title}`);
    const listingId = row.id;
    // position 0 is the cover image; the rest follow in array order.
    for (const [position, url] of input.images.entries()) {
        await client.query(`INSERT INTO listing_images (listing_id, url, "position") VALUES ($1, $2, $3)`, [listingId, url, position]);
    }
    return listingId;
}
...
async function main() {
    const passwordHash = await (0, bcryptjs_1.hash)(SEED_PASSWORD, BCRYPT_COST);
    await (0, db_1.withTransaction)(async (client) => {
        ...
        await insertListing(client, {
            ...
            description: 'Battery health 87%. Always in a case, screen has no scratches. Comes with the original box and a USB-C cable.',
            price: '299.00',
            condition: 'good',
            city: 'Portland',
        ...
        await insertListing(client, {
            ...
            description: 'i7-1165G7, 512GB SSD. Bought for a job that went remote-first; barely opened. Linux and Windows both boot fine.',
            price: '640.00',
            condition: 'like_new',
            city: 'Portland',
        ...
        await insertListing(client, {
            ...
            description: 'Classic steel commuter. New tyres and chain last spring. Some paint chips on the top tube, everything else is solid.',
            price: '700.00',
            condition: 'like_new',
            city: 'Portland',
        ...
        await insertListing(client, {
            ...
            description: 'Screen is perfect, back glass is spidered in one corner. Everything works. Priced to move.',
            price: '180.00',
            condition: 'fair',
            city: 'Seattle',
        ...
        await insertListing(client, {
            ...
            description: 'The good keyboard generation. AppleCare expired last year. Minor wear on the palm rest, battery cycle count 340.',
            price: '750.00',
            condition: 'good',
            city: 'Seattle',
        ...
        await insertListing(client, {
            ...
            description: 'Took a swim. Screen and camera module are almost certainly fine. Sold as-is, no returns.',
            price: '45.00',
            condition: 'for_parts',
            city: 'Seattle',
        ...
        await insertListing(client, {
            ...
            description: 'Posture-fit lumbar, adjustable arms, the works. Bought at an office liquidation. Mesh is taut, no sagging.',
            price: '480.00',
            condition: 'good',
            city: 'Portland',
        ...
        await insertListing(client, {
            ...
            description: 'Heavy. You will need two people and a van. Water rings on one end, which is why it is cheap.',
            price: '220.00',
            condition: 'fair',
            city: 'Portland',
    ...
    const { rows } = await db_1.pool.query(`SELECT (SELECT count(*) FROM listings)   AS listings,
