...
exports.counterpartyIds = counterpartyIds;
 ...
 */
async function thread(userId, listingId, otherUserId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT id, listing_id, sender_id, recipient_id, body, is_read, created_at
     FROM messages
     WHERE listing_id = $2
       AND ((sender_id = $1 AND recipient_id = $3)
         OR (sender_id = $3 AND recipient_id = $1))
     ORDER BY created_at, id`, [userId, listingId, otherUserId]);
    return rows.map((row) => ({
        id: row.id,
...
async function insert(senderId, recipientId, listingId, body, db = db_1.pool) {
    const { rows } = await db.query(`INSERT INTO messages (listing_id, sender_id, recipient_id, body)
     VALUES ($1, $2, $3, $4)
     RETURNING id, listing_id, sender_id, recipient_id, body, is_read, created_at`, [listingId, senderId, recipientId, body]);
    const row = rows[0];
    if (!row)
 ...
 */
async function listThreads(userId, db = db_1.pool) {
    const { rows } = await db.query(`WITH conversations AS (
       SELECT m.*,
 ...
 * else's unread badge.
 */
async function markRead(userId, listingId, otherUserId, db = db_1.pool) {
    const { rowCount } = await db.query(`UPDATE messages
        SET is_read = TRUE
      WHERE listing_id = $2
        AND recipient_id = $1
        AND sender_id = $3
        AND is_read = FALSE`, [userId, listingId, otherUserId]);
    return rowCount ?? 0;
}
/**
 ...
 */
async function counterpartyIds(userId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT DISTINCT
            CASE WHEN sender_id = $1 THEN recipient_id ELSE sender_id END AS id
       FROM messages
      WHERE sender_id = $1 OR recipient_id = $1`, [userId]);
    return rows.map((row) => row.id);
}
/** Total unread messages addressed to this user, across every thread. */
async function unreadCount(userId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT count(*)::int AS count
     FROM messages
     WHERE recipient_id = $1 AND is_read = FALSE`, [userId]);
