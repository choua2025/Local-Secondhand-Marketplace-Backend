...
exports.claimListingForPurchase = claimListingForPurchase;
exports.insert = insert;
exports.findByIdWithParties = findByIdWithParties;
 ...
 */
async function claimListingForPurchase(listingId, db) {
    const { rows } = await db.query(`UPDATE listings
        SET status = 'pending', updated_at = now()
      WHERE id = $1 AND status = 'active'
      RETURNING seller_id, price`, [listingId]);
    return rows[0] ?? null;
}
/** `amount` is a snapshot: later edits to the listing price must not rewrite history. */
async function insert(buyerId, listingId, amount, db) {
    const { rows } = await db.query(`INSERT INTO orders (listing_id, buyer_id, amount)
     VALUES ($1, $2, $3)
     RETURNING id, listing_id, buyer_id, amount, status, created_at, completed_at,
               (SELECT seller_id FROM listings WHERE id = $1) AS seller_id`, [listingId, buyerId, amount]);
    const row = rows[0];
    if (!row)
...
async function findByIdWithParties(orderId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT o.id, o.listing_id, o.buyer_id, o.amount, o.status, o.created_at, o.completed_at,
            l.seller_id
     FROM orders o
 ...
 */
async function listForUser(userId, db = db_1.pool) {
    const { rows } = await db.query(`SELECT o.id, o.listing_id, o.amount, o.status, o.created_at, o.completed_at,
 ...
 * alone otherwise.
 */
async function updateStatus(orderId, expectedCurrent, next, db) {
    ...
    const { rowCount } = await db.query(`UPDATE orders
        SET status = $3::order_status,
            completed_at = CASE WHEN $3::order_status = 'completed'
                                THEN now() ELSE completed_at END
      WHERE id = $1 AND status = $2::order_status`, [orderId, expectedCurrent, next]);
    return (rowCount ?? 0) > 0;
}
