...
export async function insert(
  ...
  const { rows } = await db.query<ReviewDto>(
    `WITH inserted AS (
     ...
     FROM inserted
     JOIN users u ON u.id = inserted.reviewer_id`,
    [orderId, reviewerId, revieweeId, rating, body],
  );
  const row = rows[0];
...
export async function listForReviewee(
  revieweeId: number,
  db: Queryable = pool,
): Promise<{ reviews: ReviewDto[]; average: number | null; count: number }> {
  const { rows } = await db.query<ReviewDto>(
    `SELECT r.id, r.order_id, r.reviewer_id, r.rating, r.body, r.created_at,
