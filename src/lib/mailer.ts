...
export interface PasswordResetEmail {
  to: string;
  /** The full link the user clicks, token included. */
  resetUrl: string;
  /** The raw token, so a transport can render it separately if it wants to. */
  token: string;
}

export type Transport = (email: PasswordResetEmail) => Promise<void> | void;

const consoleTransport: Transport = (email) => {
  // Never in production: this line puts a live credential in the log stream.
  if (process.env.NODE_ENV === 'production') {
    console.warn(
      `[mailer] No transport configured — the reset email for ${email.to} was NOT sent.`,
    );
    return;
  }
  console.log(
...
export function initMailerFromEnv(): void {
  ...
  if (!host) {
    if (process.env.NODE_ENV === 'production') {
      console.warn(
        '[mailer] SMTP_HOST is not set — password-reset emails will NOT be delivered. ' +
